...
- **The daemon is `start.php`, period.** `php start.php start` is what `systemd/phlix-server.service` and `docker/supervisord.conf` run: HTTP worker `:8096` (`count = 14`), WebSocket `:8097`, plus hub-heartbeat / background-timer / relay-tunnel workers at `count = 1` (a stall there is a 100% outage of that subsystem). The former one-shot CGI front controller `public/index.php` was deleted by S171 — it ignored argv, returned immediately, and no deployment artifact served it; the removal is pinned by `tests/Unit/Docker/PublicFrontControllerRemovalGuardTest.php`.

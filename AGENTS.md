...
**Coverage threshold.** `phpunit.yml` enforces a minimum statement-coverage percentage computed from the Clover XML (`/coverage/project/metrics/@statements` and `@coveredstatements`) — NOT Cobertura's `@line-rate`, which PHPUnit does not emit. The parse lives in `scripts/coverage-threshold-check.php`, **not** inline in the workflow, and that placement is load-bearing: `xmllint` and `bc` are both absent from the `ubuntu-latest` (Ubuntu 24.04) runner image, so the step's original `xmllint … 2>/dev/null || echo "0"` resolved to `0` on every run, took its `exit 0` "no statements found — skipping threshold check" branch, and gated nothing on any PR or push until this was repaired on 2026-07-28. The XPath was never wrong. Actual coverage at the time of the repair was **65.15% (40711/62486)**; the floor is `MIN_COVERAGE=40`, declared in the step's `env:`. Bump it as coverage grows; never set it above current coverage or every PR turns red. Every failure mode in the checker is `exit 1` — missing report, missing parser, unparseable XML, missing/non-numeric metric, zero statements — because a gate that cannot measure must not report success (the same defect class as S146's permanently-green Psalm job). `tests/Unit/Support/CoverageThresholdCheckTest.php` is what keeps it that way; do not add an `exit 0` fallback.

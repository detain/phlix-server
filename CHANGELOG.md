...
- **WS-A — Continue-Watching now shows poster images and progress bar in /app SPA.**
  Episode rows now correctly resolve series poster via `MediaItemShaper::shape()` instead of showing TMDB still frames. Rows are shaped with top-level `id`, `poster_url`, `runtime` (minutes), `position_ticks`, `duration_ticks`, `media_item_id`, and `metadata`. Joins parent series to get real series poster for episodes; falls back to season poster. Tests in `tests/Unit/Session/PlaybackControllerTest.php`.
- **WS-C — Logging no longer triple-writes to app.log, events.log, plugins.log.**
  Handlers in `config/logger.php` are now tagged with `channels` and `env` gates. `handlerAppliesToChannel()` routes `EVENTS` channel to `events.log` (guarded by `PHLIX_DEBUG_EVENTS`) and `PLUGINS` channel to `plugins.log`. `app.log` and `error.log` remain unrestricted, capturing everything. Tests in `tests/Unit/Common/Logger/StructuredLoggerTest.php`.

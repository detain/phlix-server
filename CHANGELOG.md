...
- **item-5b — logger handler routing (3× write-amplification + subsystem-log pollution).** `StructuredLogger::setupHandlers()` previously attached **every** `config/logger.php` handler to **every** channel, so each debug/info record was written to `app.log` + `events.log` + `plugins.log` (3×), and the two subsystem logs were polluted with unrelated application records. Handlers now honor two optional gates: `channels` (a list; absent/empty = all channels) and `env` (an env var that must be truthy; absent = always). `events.log` is scoped to the `EVENTS` channel **and** gated on `PHLIX_DEBUG_EVENTS` (the promise the config always documented but never enforced); `plugins.log` is scoped to the `PLUGINS` channel. `app.log` and `error.log` remain unrestricted — the general app log still captures everything and error aggregation still spans all channels, so no diagnostic coverage is lost. No change to *what* is logged per level, only *which files* each channel lands in.
...
- **item-5c3 — enabled plugins did not re-attach their event listeners after a restart.** `PluginLoader::bootstrapEnabled()` existed but was never called in any resident boot path, so PSR-14 event listeners (e.g. Trakt PUSH scrobbling on `PlaybackStarted`/`PlaybackStopped`) went dead after every restart until a manual re-enable. It is now invoked per-worker in both the HTTP and relay-tunnel `onWorkerStart` (the event dispatcher is per-worker), guarded so a broken plugin can never stop a worker serving. The call is idempotent (the `entryInstances` guard + `ListenerRegistry` duplicate-subscription guard), and the worker-0-gated Trakt **pull**-sync timer is orthogonal (it uses a fresh instance that never subscribes), so there is no double-attach. The single-shot CGI/FPM `public/index.php` path deliberately does **not** call it (throwaway per-request container).

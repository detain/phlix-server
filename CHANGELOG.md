...
- Continue-Watching in `/app` now shows poster images and progress bar (episodes correctly resolve series poster via `MediaItemShaper::shape()`; rows now carry top-level `id`, `poster_url`, `runtime`, `position_ticks`)

...
- **`FfmpegRunner::buildSegmentCommand()`: per-rung capped-CRF encode + genuine stream-copy passthrough (Stream Quality/ABR step A4) — `FfmpegRunner`-only groundwork; the copy path is DORMANT until step A5 wires a real per-rung/Original decision into `TranscodeManager::computeSegmentParams()` (which still forces copy→re-encode today, so there is no visible behavior change in production yet).** Two segment shapes, selected per-stream from the caller's params:

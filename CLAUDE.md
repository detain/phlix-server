...
php start.php start                         # the ONLY served entry (HTTP :8096, WS :8097)
...
PHP 8.3+ media server on **Workerman 5.x**. PSR-4 autoload: `Phlix\` → `src/`, `Phlix\Tests\` → `tests/`. The **only served entry point is `start.php`** at the repo root (the single-process CGI/FPM bootstrap `public/index.php` was unserved by every deployment artifact and deleted by S171; removal pinned by `tests/Unit/Docker/PublicFrontControllerRemovalGuardTest.php`) — `Worker::runAll()` over an HTTP worker on `:8096` (count 14), a WebSocket worker on `:8097`, plus hub-heartbeat, background-timer and relay-tunnel workers (each count 1) — building its routes through `src/Server/Core/Application.php`.

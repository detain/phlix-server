...
- [ ] SV-4.15 port hub's per-surface rate-limiting framework to server's unprotected auth surfaces — QUEUED 2026-07-13 (perf-7, cross-repo consistency gap). Server only rate-limits `login` (DB-backed, DbLoginRateLimitStore); `register`/`refresh`/WebAuthn start+finish/public JWKS/WS-connect (:8097) have NO rate limiting, unlike hub's general `Common/RateLimit/` framework. Full spec in `/home/sites/phlix/performance_plan.md` §2 S-W4 SV-4.15. **⚠️ GATED (user direction 2026-07-13): do NOT start until hub's rate-limiting work is FULLY finished + reviewed + docs'd** — HB-4.6 Option B (DbRateLimiter + LOGIN repoint) was still in progress when this was drafted; re-audit the hub donor code fresh once it's settled (don't trust this spec's description of hub's shape as final). Also gated behind the rest of the server COMPLETE queue per the plan's existing ordering.
  ...
  - **SV-4.15(b)+(c) DONE 2026-07-18** — (b) added `RateLimitException::retryAfterSeconds(?int $now=null): int<0,max>` (`src/Auth/RateLimitException.php`), delegating `max(0,resetAt-now)` to the ported `RateLimitState::retryAfter()` (mirrors hub). (c) central `RateLimitException`→429 mapping at BOTH dispatch entrypoints via a NEW shared static helper `Application::rateLimitResponse()` (`src/Server/Core/Application.php`) = 429 + `Retry-After: <retryAfterSeconds()>` + JSON `{error:"Too Many Requests",code:"rate_limited"}`: Workerman `HttpHandler.__invoke` catches `RateLimitException` immediately before the generic `catch(Throwable)`; `Application::handleException()` (the `run()` CGI path) short-circuits it; `public/index.php` registers a `set_exception_handler` that maps it (re-throws all others → status quo). **This fixes the latent login-500 bug** — the existing `DbLoginRateLimitStore`→`AuthManager` trip was uncaught by `AuthController::login` and 500'd with no Retry-After; now 429. Tests (+12): `tests/Unit/Auth/RateLimitExceptionTest.php` (6: floor-at-0, default-to-time, exact-seconds), `tests/Unit/Server/Core/ApplicationRateLimitResponseTest.php` (3: envelope), `tests/Unit/Server/Workerman/HttpHandlerRateLimitTest.php` (3: dispatch-throw→429, **login-limiter-trip→429-not-500 regression** driving real AuthController→AuthManager→DbLoginRateLimitStore over-limit through `__invoke`, controller-doesn't-swallow). Full Unit **5808 / 42231 assertions / 0 fail / 0 err / 5 skip** (5796 + 12). phpstan L9 clean; phpcs 0/0 on changed src (public/index.php's 5 long-line warnings are pre-existing debug `error_log` lines, not my additions). **Sub-steps d–h PENDING** (d=RateLimitProfiles, e=DbRateLimiter+positional-`?`+migration, f/g/h=per-surface wiring: register/refresh/WebAuthn/JWKS/WS-connect).
  - **SV-4.15(e) DONE 2026-07-18** — new SHARED, DB-backed generic limiter `src/Common/RateLimit/DbRateLimiter.php` (`implements RateLimiterInterface`; ctor `(Connection $db, int $windowSeconds=900, int $maxAttempts=5, ?callable $clock=null)`; opaque `rate_key`) — the cross-worker-true-global backend for brute-force surfaces (in-memory gives 14× budget). Modeled on `DbLoginRateLimitStore` (positional `?`, `INSERT … ON DUPLICATE KEY UPDATE attempts=IF(reset_at<=?,1,attempts+1), reset_at=IF(reset_at<=?,?,reset_at)` atomic single-statement upsert → re-SELECT → `RateLimitState(limited=attempts>=max)`; `peek` read-only SELECT; `reset` DELETE by rate_key; bounded `sweepExpired` `DELETE … WHERE reset_at<=? LIMIT <int>` with native-int LIMIT = the 1064 defense), NOT hub's named-`:param` SQL. New table via `migrations/085_rate_limit_buckets.sql` (`rate_key VARCHAR(191) PK, attempts INT UNSIGNED, reset_at INT UNSIGNED, created_at INT UNSIGNED, INDEX idx_reset_at`, InnoDB utf8mb4, `CREATE TABLE IF NOT EXISTS` idempotent) — SEPARATE from mig 074's ip-keyed `login_rate_limit`; `DbLoginRateLimitStore`/mig 074/`login` surface UNTOUCHED. Tests (+11): `tests/Unit/Common/RateLimit/DbRateLimiterTest.php` — behavioural positional-`?` fake DB modeling the table (fresh→1/not-limited, increment-within-window, restart-after-expiry, distinct-key isolation, `limited` at max, `peek` no-write, `reset` DELETE, upsert+bounded sweep, sweep LIMIT/threshold bound as INT (1064 guard), positional-not-named placeholders, non-positive clamp-to-1). Full Unit **5819 / 42276 assertions / 0 fail / 0 err / 8 skip** (5808 + 11; skips env-dependent). phpstan L9 clean; phpcs 0/0 (src + changed). **Sub-steps d, f/g/h PENDING** (d=RateLimitProfiles + config/DI; f/g/h=per-surface wiring: register/refresh/WebAuthn/JWKS/WS-connect).
  - **SV-4.15(d) DONE 2026-07-18** — per-surface profile catalogue + config + DI registration (NO surface wiring yet — that's f/g/h). New `src/Common/RateLimit/RateLimitProfiles.php` (namespace `Phlix\Common\RateLimit`, adapted from hub's): 6 typed container-id consts `REGISTER`/`REFRESH`/`WEBAUTHN_START`/`WEBAUTHN_FINISH`/`JWKS`/`WS_CONNECT` (`rate_limiter.<surface>`), `defaults()` map id→`{key,max,window}` (register 5/600, refresh 30/60, webauthn_start 10/60, webauthn_finish 10/60, jwks 120/60, ws_connect 30/60) — DELIBERATELY no `login` profile (login keeps `DbLoginRateLimitStore`/mig 074). Backend classification `dbBacked()`/`isDbBacked()`: register/refresh/webauthn_start/webauthn_finish → DB (true-global across 14 HTTP workers), jwks/ws_connect → in-memory (jwks cache-frontable; :8097 WS worker is count=1 so per-worker==global). Config: new `rate_limit` block in `config/server.php` — iterates `RateLimitProfiles::defaults()` and reads bare-`getenv()` overrides `RATE_LIMIT_<SURFACE>_MAX` / `_WINDOW` (server convention, mirrors hls block's HLS_* knobs), falling back to `defaults()`. DI: registered in **`AuthServicesProvider`** (server has NO `CommonServicesProvider`; this is where `DbLoginRateLimitStore`/`AuthManager` are wired) via new private `registerRateLimiters()` — one explicit `factory()` per profile id (NOT autowiring): DB-backed → `new DbRateLimiter($db,$window,$max)` (Connection resolved via `Connection::class`, the SAME pooled `mysql` conn `DbLoginRateLimitStore` uses — readily injectable since a `factory(fn(Connection $db)=>…)` param autowires from the container binding in CoreServicesProvider; NOT a dedicated txn conn), in-memory → `new RateLimiter($window,$max)`; `$window`/`$max` from the `rate_limit` config block per profile. No `RateLimiterInterface::class` alias registered (nothing in server src injects the bare interface — grep-confirmed; f/g/h will inject the named profile ids). Tests (+12): `tests/Unit/Common/RateLimit/RateLimitProfilesTest.php` (6: exactly-6-surfaces/no-login, id namespacing, defaults shape+thresholds, unique keys derived from ids, dbBacked subset, isDbBacked incl. unknown-id→false) + `tests/Unit/Common/Container/Providers/AuthServicesProviderRateLimitTest.php` (6: DB surfaces→DbRateLimiter, in-memory→RateLimiter, distinct instances, DB-config-override applied, in-memory-config-override applied, all-profiles-resolve+no-login-profile; max via `peek()->limit`, window via fresh `hit()->resetAt` with mock Connection). Full Unit **5831 / 42393 assertions / 0 fail / 0 err / 5 skip** (5819 + 12; skips env-dependent). phpstan L9 clean; phpcs 0/0 (changed src + config + tests). **Sub-steps f/g/h PENDING** (per-surface wiring: register/refresh → AuthController, WebAuthn start/finish → WebAuthnController, JWKS → HubJwksController, WS-connect → WebSocketServer::onWebSocketConnect).
  - **SV-4.15(f) DONE 2026-07-18** — wired the 4 auth surfaces to their per-surface limiters (JWKS/WS-connect still PENDING = g/h). Pattern per surface: `hit($key)`; if `$state->limited` throw `RateLimitException($state->resetAt, $state->remaining)` BEFORE the real work → central 429 mapping (sub-step c). Keys use `Request::getClientIp()` (X-Forwarded-For aware) NOT the stale `$_SERVER`/`AuthManager::getClientIp()`. **`AuthController`** (`src/Server/Http/Controllers/AuthController.php`): ctor gains 2 OPTIONAL `?RateLimiterInterface` params (`registerLimiter`,`refreshLimiter`, default null → private `enforceRateLimit()` no-op) so the ~20 existing `new AuthController($authManager)` test sites + the degraded no-container `Application::getAuthController()` fallback keep working; `register()` keyed `register:<ip>`, `refresh()` keyed `refresh:<ip>`. **`WebAuthnController`** (`src/Server/Http/Controllers/WebAuthnController.php`): ctor gains 2 optional `?RateLimiterInterface` params (`startAuthLimiter`,`finishAuthLimiter`); both ceremonies key on the SUBMITTED USERNAME (available in the request body at both limit points — `webauthn_start:<username>` / `webauthn_finish:<username>`), falling back to `<ip>` via `limitIdentifier()` only when username absent/blank (start's IP-fallback also covers the missing-field 400 path). **DI** (`src/Common/Container/Providers/AuthServicesProvider.php`): the optional params are bound EXPLICITLY (PHP-DI optional-param landmine — an unbound optional stays null) — `AuthController::class` now registered `autowire()->constructorParameter('registerLimiter', get(RateLimitProfiles::REGISTER))->constructorParameter('refreshLimiter', get(REFRESH))` (was pure-autowire), and `WebAuthnController::class` autowire gains `startAuthLimiter`→`get(WEBAUTHN_START)` / `finishAuthLimiter`→`get(WEBAUTHN_FINISH)`; all 4 resolve to the shared DB-backed `DbRateLimiter` per sub-step d/e. Tests (+17): `tests/Unit/Server/Http/Controllers/AuthControllerRateLimitTest.php` (6: register/refresh over-limit→throw→`Application::rateLimitResponse()` 429+Retry-After+`code=rate_limited`; under-limit→proceeds; exact key asserted; getClientIp-not-$_SERVER source proof; null-limiter no-op), `tests/Unit/Auth/WebAuthn/WebAuthnControllerRateLimitTest.php` (6: start/finish over/under-limit, username-keyed, IP-fallback when username absent, null no-op), `tests/Unit/Common/Container/Providers/AuthServicesProviderControllerWiringTest.php` (5: container-resolved controllers trip when the injected DB limiter reports over-limit + injected limiters `===` the registered profile instances). Full Unit **5848 / 42453 assertions / 0 fail / 0 err / 5 skip** (5831 + 17). phpstan L9 clean; phpcs 0/0 on changed src. **Sub-steps g/h PENDING** (JWKS → `HubJwksController`, WS-connect → `WebSocketServer::onWebSocketConnect`).
  - **SV-4.15(g)+(h) DONE 2026-07-18 — SV-4.15 surface wiring COMPLETE (a–h all wired).** (g) JWKS: `HubJwksController::handle` (`src/Server/Http/Controllers/HubJwksController.php`) gains an OPTIONAL `?RateLimiterInterface $limiter` ctor param (null = no-op, so the degraded no-container `Application::getHubJwksController()` fallback keeps working); before serving keys it does `hit('jwks:'.$request->getClientIp())` and, if `limited`, throws `RateLimitException($state->resetAt,$state->remaining)` → central 429 (sub-step c) — the inline-closure route at `Application.php:295` is UNCHANGED (it just delegates to `handle()`, so the throw propagates through the same dispatch the central mapping catches). In-memory/per-worker is deliberate (public, cache-frontable DoS surface). DI: `HubServicesProvider` binds `limiter`→`get(RateLimitProfiles::JWKS)` EXPLICITLY (PHP-DI optional-param landmine); the JWKS profile is registered in `AuthServicesProvider` (providers merge into one container). (h) WS-connect: `WebSocketServer` (`src/Server/WebSocket/WebSocketServer.php`) gains a nullable `wsConnectLimiter` field + `setWsConnectLimiter()` setter (matches the existing `setSyncPlayManager`/`setMetricsCollector` injection style; null = unthrottled so all direct-construction/test sites keep working). At the TOP of `onWebSocketConnect`, AFTER the `findConnection` null-guard but BEFORE the `authMiddleware===null` auth gate, it does `hit('ws_connect:'.$connection->getRemoteIp())`; on `limited` it logs a warning + mirrors the EXISTING reject idiom (`$this->connections->remove($id)` + `$connection->close()`) + `return` — **INLINE, never throws** (a throw out of the hook triggers Workerman `Worker::stopAll()` — verified `vendor/.../Protocols/Websocket.php:440`). **Close code:** plain `close()` (NOT WS 1013) is correct here because the hook fires BEFORE the 101 upgrade response (Websocket.php:440 vs :459), so the socket isn't a WebSocket at the byte layer yet and a raw 1013 close frame would be malformed/out-of-order; a TCP close is the right rejection — spec allows the existing-close fallback. :8097 WS worker is `count=1` so the in-memory limiter is already server-wide (no DB). Injection point: `start.php` WS `onWorkerStart` resolves `RateLimitProfiles::WS_CONNECT` from the per-worker container (best-effort try/catch → unthrottled on failure, never blocks the listener) and calls `setWsConnectLimiter()`. Tests (+9): `tests/Unit/Server/Http/Controllers/HubJwksControllerRateLimitTest.php` (3: over-limit→429+Retry-After+`rate_limited` & `getPublicKeysJwk` never called & key `jwks:<XFF-ip>`; under-limit→200 JWKS body; null no-op), `tests/Unit/Server/WebSocket/WsConnectRateLimitTest.php` (4: over-limit→removed+closed & valid token NOT authenticated = before-auth proof & key `ws_connect:<remoteIp>`; under-limit→auth reached+authenticated; over-limit rejects anonymous/dev no-secret path too; no-limiter unthrottled), `tests/Unit/Common/Container/Providers/HubServicesProviderJwksWiringTest.php` (2: container-injected limiter `===` registered JWKS profile; container-wired limiter enforces end-to-end via jwks max=2). Full Unit **5857 / 42488 assertions / 0 fail / 0 err / 5 skip** (5848 + 9). phpstan L9 clean; phpcs 0/0 on changed src (start.php's one long-line warning is the PRE-EXISTING HWACCEL_DEBUG `error_log`, not mine). **STILL OWED (orchestrator next):** a cumulative a–h review + an on-box verification of the JWKS 429 (Retry-After) and the WS-connect close (1013 intentionally NOT sent — pre-101 TCP close).
...
- SV-4.8 Router static-path fast map + DI — **DONE** (real O(1) $staticRoutes map consulted before regex; string handlers resolved via container->get, index.php path only registers instances so never hits `new $class()`). GAP: DI-resolution branch + static-map-served assertion untested.
  - **TestEngineer — 2026-07-18 — GREEN (gap closed).** Added 5 tests to `tests/Unit/Server/Http/RouterTest.php` (+ fixture `tests/Unit/Server/Http/Fixtures/RouterFixtureController.php`): (1) DI branch — mock PSR-11 `ContainerInterface`, `expects(once)->get(RouterFixtureController::class)` returns a spy, asserts the container-returned instance is the one invoked; (2) fallback branch — no container → `new $class()` (marked Response can only come from direct instantiation); (3) static-map fast path — a `/{slug}` parametric route registered alongside `/fast`; its closure never runs on the `/fast` static hit, proving the regex loop is skipped regardless of registration order; (4) HEAD via static map — HEAD `/health` (GET-only static) resolves through `dispatchAsHead()` static lookup, `headOnly=true` (a parametric GET route is also registered to satisfy the `isset(routes['GET'])` fallback guard); (5) BadMethodCallException on the array/DI path — resolved handler returning a non-Response throws. RouterTest 11/11 green. Full Unit **5725/0 fail/0 err/8 skip** (5720 baseline +5). phpstan L9 `[OK] No errors`; phpcs PSR12 clean on both changed test files + config. LATENT EDGE-CASE (not fixed, harmless in prod): the HEAD→GET fallback guard checks only `isset($this->routes['GET'])` (parametric map); if a GET path existed ONLY as a *static* route with ZERO parametric GET routes registered, HEAD to it would 404 instead of reaching `dispatchAsHead()`'s static branch. Prod always has parametric GET routes so it never triggers; flagged for a future guard tweak (`|| isset($this->staticRoutes['GET'])`).
  - **TestEngineer — 2026-07-18 — HARDENED (latent edge fixed).** Applied the flagged guard tweak in `src/Server/Http/Router.php` `dispatch()`: both HEAD→GET fallback guards now read `isset($this->routes['GET']) || isset($this->staticRoutes['GET'])`, and `dispatchAsHead()`'s parametric loop is `foreach (($this->routes['GET'] ?? []) as ...)` so a static-only GET map can't warn on a null foreach. Added 2 RouterTest cases: `testHeadResolvesStaticOnlyGetRouteWithNoParametricGetRoutes` (register `/ping` as the SOLE GET route → purely static, `routes['GET']` unset → HEAD `/ping` now resolves 200 + headOnly instead of 404) and `testHeadStillReturns404WhenNoGetRouteMatchesPath` (normal case unchanged — HEAD to an unregistered path still 404s). Separate commit from the SV-4.12 tests.
...
- SV-4.12 stale-job reaper glob — **DONE** (reapStaleRunningJobs :2913-2916 merges chunk-*.m4s + seg-*.ts). GAP: seg-*.ts keep-branch untested.
  - **TestEngineer — 2026-07-18 — GREEN (gap closed).** Added 3 tests to `tests/Unit/Media/Transcoding/TranscodeManagerTest.php` pinning the `glob("$dir/seg-*.ts")` arm (deleting that line was previously inert to the suite): (1 load-bearing) wedged-age running job whose dir holds ONLY `seg-v720p-00000.ts` → NOT reaped (`assertSame(0)`); (2) wedged-age job with ONLY `seg-*.ts.part-*` temps (no finalized `.ts`, no chunk) → REAPED with 'no segment produced within' (the `.ts` glob must not match `.part-*` suffixes); (3) mixed dir with BOTH `chunk-*.m4s` + `seg-*.ts` → kept (locks the `array_merge` of both arms). Impl unchanged. Full Unit suite **5730 / 41882 assertions / 0 failures / 0 errors / 5 skipped**; phpstan L9 `[OK] No errors`; phpcs PSR12 clean on changed files.
...
- SV-0.6 TMDB collections UUID-as-int (ad6d6d86) — **DONE (code).** syncCollectionForMovie(string)/removeCollectionMembership(string); MediaScanner:1383 (int) cast GONE; other (int) casts are on real tmdb_id/part.id (correct). GAP: NO tests at all (no CollectionServiceTest; plan-mandated UUID-not-"0" unit + membership integration missing).
  - **CLOSED (2026-07-18, 53d8dcc2 + b42cacf2):** tests built out — `tests/Unit/Collections/CollectionServiceTest.php`, 9 cases, 100% of syncCollectionForMovie/removeCollectionMembership (findById gets exact UUID not "0"; membership INSERT binds UUID + tmdb_part_order; no-collection/no-tmdb-id/unresolved paths). PART A latent-bug FIX: `MediaScanner.php:1385` `syncCollectionForMovie($itemId,'')` → `(string) $itemId` (every sibling call already casts; strict_types string param would TypeError on an int and the catch swallows it — silent re-regression). Full Unit suite green: 5677 tests / 41775 assertions / 0 failures / 0 errors / 5 skipped; phpstan L9 clean; phpcs 0/0.
- SV-0.7 marker worker supervision (46c71440) — **DONE.** config/process.php:54-58 marker-detection enabled count=1 poll=30; start.php:606/610-649 launches BackgroundDetectorWorker under runAll; ::start arms Timer→runOnce→dequeueShow. GAP: no config test + no enqueue-smoke test.
  - **CLOSED (2026-07-18):** tests built out — 3 config guards added to `tests/Unit/Config/ManagedWorkersConfigTest.php` (marker-detection registered+enabled with count/poll_seconds; map→BackgroundDetectorWorker::class; poll_seconds===marker_detection.php worker_interval coherence pin) + new `tests/Unit/Media/Markers/Detection/BackgroundDetectorWorkerTest.php` (4 cases): enqueue→drain smoke via real temp-dir MarkerCandidateStore with stubbed IntroDetectionJob/MarkerCandidateRepository (runOnce drains→getPendingCount 0), empty-queue→false, stores-candidates-when-markers, and detector-throws→caught+queue-advances failure path. Impl unchanged (no bug found). Full Unit suite green: 5684 tests / 41801 assertions / 0 failures / 0 errors / 5 skipped; phpstan L9 clean; phpcs clean on new/changed test files.
...
#### TestEngineer — 2026-07-18 (SV-1.8 test top-up) — GREEN
New `tests/Unit/Server/Http/RequestAuthenticatorTest.php` (29 tests, 39 assertions): suffix bypass rejected both directions (`evil-example.com`, `example.com.evil.com`); exact same-origin accepted; port match accepted / mismatch + asymmetric-port both-ways rejected; Referer enforced identically (match/mismatch/unparseable); both-absent rejected; unparseable Origin rejected; Origin `''`/`'null'` skipped but Referer still enforced; safe methods (GET/HEAD/OPTIONS) allowed; malformed Host fail-safe (parseHostHeader fallback). Gate-level via `HttpHandler::__invoke` (mock TcpConnection): cookie-auth POST + bad Origin → **403 body `code=csrf.invalid_origin`**; bearer-auth POST + bad Origin → **bypasses gate (dispatches, 200)**. Coverage RequestAuthenticator.php **60/60 stmts (100%), 6/6 methods**. Verification (real output): `RequestAuthenticatorTest` = OK (29 tests, 39 assertions); full Unit = `Tests: 5713, Assertions: 41840, Skipped: 5` (0 fail/0 err; 5684 baseline + 29); phpstan L9 `[OK] No errors`; phpcs PSR12 exit 0 on the new file. Impl audit verdict: implementation correct, no production changes. NOTE: plan AC (`evil-example.com` rejected, host:port works, same-origin passes) is fully met by exact same-origin compare; a configured trusted-origins allowlist remains unimplemented (low-priority, NOT required by AC — not added). OBSERVATION (not a bug given the threat model): `Origin: ''` with NO Referer on a state-changing request returns true (empty-string origin skipped, both-absent early-check keyed on PHP `null` only) — browsers never emit an empty-string Origin cross-site, so no realistic CSRF vector; flagged for awareness, left as-is.
- SV-1.9 ENOSPC guard — **DONE (code+wired).** TranscodeManager:1461-1484 ensureDiskSpace (disk_free_space vs minDiskSpaceBytes default 500MiB) called before startSegmentEncode @812/@1032; HlsController:147-156 catches SegmentCacheFullException→sweep+503+Retry-After. GAPS: ZERO ENOSPC tests; threshold NOT surfaced in config (TranscodeServicesProvider doesn't pass minDiskSpaceBytes; hardcoded default). → TEST + optional config key.
  - **TestEngineer/Implementer — 2026-07-18 — GREEN (gaps closed).** PART A config-wiring: added `hls.min_disk_space_bytes` (env `HLS_CACHE_MIN_FREE_BYTES`, default 500 MiB) to `config/server.php` next to its sibling segment-cache knobs (cache_max_bytes/cache_max_age) — that's where the provider reads `$appConfig['hls']`; `config/transcoding.php` is NOT exposed in `$appConfig` (only read by HwAccelConfig), so `['hls']` is the convention-consistent home. `TranscodeServicesProvider` now reads it and threads it to ctor **position 13** (`$minDiskSpaceBytes`); because args are positional, **position 12** (`$segmentMaxWaitMs`) is now passed as explicit `null` to preserve its existing default (SEGMENT_MAX_WAIT_MS) verbatim — no behavior change when the key is absent (null → 500 MiB ctor default). PHP-DI "skips optional params" landmine N/A: the manager is built by an explicit `factory()` closure, not autowiring. PART B tests (+7, all green): HlsControllerTest ×2 (legacy `seg-` + multi-variant `seg-v` routes → 503 + Retry-After **'3'** + `sweepSegmentCache()` once); TranscodeManagerTest ×3 (PHP_INT_MAX floor → SegmentCacheFullException before ffmpeg spawn; 1-byte floor → encode proceeds; bogus segment_dir → disk_free_space false → fail-open encode); TranscodeServicesProviderTest ×2 (absent key → 500 MiB default; set key → threaded through, read via reflection). Verification: targeted 3 files OK (153 tests); **full Unit 5720/0 fail/0 err/8 skipped** (5713 baseline +7); phpstan L9 `[OK] No errors`; phpcs PSR12 -n clean on `src/` provider + config. Files: `config/server.php`, `src/Common/Container/Providers/TranscodeServicesProvider.php`, `tests/Unit/Server/Http/Controllers/HlsControllerTest.php`, `tests/Unit/Media/Transcoding/TranscodeManagerTest.php`, `tests/Unit/Common/Container/Providers/TranscodeServicesProviderTest.php`.
  - **TestEngineer — 2026-07-18 — env-var rename (review nit).** Renamed the brand-new SV-1.9 env override in `config/server.php` from `HLS_CACHE_MIN_FREE_BYTES` → **`HLS_MIN_DISK_SPACE_BYTES`** so it mirrors its config key `min_disk_space_bytes` the way siblings do (`cache_max_bytes`↔`HLS_CACHE_MAX_BYTES`, `max_concurrent_segments`↔`HLS_MAX_CONCURRENT_SEGMENTS`). grep confirmed the old name appeared ONLY in `config/server.php` (the provider test reads the `min_disk_space_bytes` config KEY directly, not the env var, so no test/doc referenced it) — safe rename, no other references to update. Left the unrelated class const `TranscodeManager::SEGMENT_CACHE_MIN_FREE_BYTES` untouched (out of scope). Full Unit still 5725/0/0/8; phpcs clean on config. Separate commit.
...
- New **`ItemRepository::getVideoStreamColorMetadata(string $mediaItemId): ?array`** (`src/Media/Library/ItemRepository.php`, placed right before `addStream`, after `getItemStreams`). Query (starts with SELECT):
  `SELECT color_space, color_transfer, color_primaries, max_luminance, avg_luminance FROM media_streams WHERE media_item_id = ? AND stream_type = 'video' ORDER BY stream_index LIMIT 1`.
  - `stream_type = 'video'` matches the scanner's tag (`MediaScanner.php:1571` `$isVideo = $codecType === 'video'`, persisted via `addStream` `stream_type`). `ORDER BY stream_index LIMIT 1` picks the FIRST video stream — same selection as `FfmpegRunner::extractColorMetadata()` (`FfmpegRunner.php:1381`, first video stream, defaults `bt2020nc/bt709/bt2020/1000.0/200.0`).
  - **Returns null** (→ probe fallback) when: no video-stream row, OR all three color-string columns are NULL (pre-073 / un-rescanned / ffprobe reported no color info — returning defaults there would look SDR and wash out genuine HDR).
  - **Byte-identity:** when populated, returns an `extractColorMetadata()`-shaped array applying the SAME per-field defaults for any individually-NULL column (`color_space ?? 'bt2020nc'`, `color_transfer ?? 'bt709'`, `color_primaries ?? 'bt2020'`, `max_luminance ?? 1000.0`, `avg_luminance ?? 200.0`). DECIMAL luminance strings cast to float. All 5 keys ALWAYS present → downstream reads stay guard-free (no `?? ''` → no phpstan L9 unreachable-branch).

### Where computeHlsParams now prefers columns (with fallback)
- `TranscodeManager::ensureHlsJob` (`src/Media/Transcoding/TranscodeManager.php`): after `getMediaItem`, resolves `$persistedColorMeta = $this->persistedVideoColorMetadata($mediaItemId)` (new private helper — lazily `new ItemRepository($this->db)`, NO constructor/DI change → loads identically under index.php + start.php) and threads it into `computeSegmentParams(..., $persistedColorMeta)` → `computeHlsParams(..., $persistedColorMeta)` (both gained a trailing `?array $persistedColorMeta = null`).
- HDR decision: `$colorMeta = $persistedColorMeta ?? $this->ffmpeg->extractColorMetadata($probe);` — columns when present (0 probes contributed), in-hand probe otherwise. The `require_hdr_tone_map` derivation (`isHdr` = `smpte2084`/`arib-std-b67` transfer; `isBt2020`) is unchanged, just fed column-sourced metadata.
- Tone-map filter (`computeSegmentParams`, the sub-step-(b) resolution): when `$persistedColorMeta !== null` uses the NEW **`FfmpegRunner::resolveToneMapFilterFromColorMeta($persistedColorMeta, $videoCodec)`** (0 probes); else the pre-(a) `resolveToneMapFilterFromProbe($probe, $videoCodec)`.
- `FfmpegRunner`: extracted `resolveToneMapFilterFromProbe`'s body into new public `resolveToneMapFilterFromColorMeta(?array $colorMeta, string $codec)`; `resolveToneMapFilterFromProbe` now delegates (`extractColorMetadata($probe)` then the colorMeta core). Behavior-preserving — (b)/(b′) public API + string output unchanged.

### Byte-identity confirmation
- The tone-map graph is decided by `isHdrColorMeta()` + config (`tone_mapping_mode`/`prefer_hdr_output`) + codec — NOT by the specific color VALUES (`buildZscaleToneMapFilter` hardcodes the graph; `buildLibplaceboToneMapFilter` reads tags off the frame — both ignore `$colorMeta`). Columns are the same values `extractColorMetadata` reads from the probe for the same file, so a scanned HDR item yields the SAME `require_hdr_tone_map` AND the SAME `tone_map_filter` string as the probe path. Proven at the FfmpegRunner level: `resolveToneMapFilterFromColorMeta(hdrMeta,'libx264') === resolveToneMapFilterFromProbe(hdrProbe,'libx264')`.

### HONEST probe-count framing (NOT overclaimed)
- Sub-step (a) makes the HDR *decision* column-sourced → **0 HDR-specific probes** for scanned items. It does **NOT** remove the single `ffmpeg->probe()` at `ensureHlsJob` (~:420) — that still runs for embedded subtitle/audio stream detection (`detectTextTracks`) regardless. So the **observable probe count per job stays at exactly 1**. Reaching a true 0 needs persisting subtitle/audio stream descriptors (a separate "S6" follow-up), OUT OF SCOPE here. Code comments + the `testEnsureHlsJobHdrDecisionFromColumnsAddsZeroProbes` test assert `probe()` == exactly 1 and `extractColorMetadata` == 0 on the column path.

### Tests (real numbers)
- `ItemRepositoryTest` (+4): persisted color → shaped array (DECIMAL→float); populated row with individually-NULL columns → SAME extractColorMetadata defaults (byte-identity); no video row → null; all-NULL pre-073 row → null.
- `TranscodeManagerTest` (+3): `testEnsureHlsJobSourcesHdrDecisionFromPersistedColorColumns` (HDR columns present + probe stub → `require_hdr_tone_map=true` sourced from columns, `extractColorMetadata`+`resolveToneMapFilterFromProbe` asserted NEVER called, `resolveToneMapFilterFromColorMeta` called once with the column-shaped meta + `libx264` → `tone_map_filter` == canon); `testEnsureHlsJobFallsBackToProbeColorWhenColumnsAbsent` (no columns → probe path, `resolveToneMapFilterFromColorMeta` NEVER called; mutation-red if column path taken when absent); `testEnsureHlsJobHdrDecisionFromColumnsAddsZeroProbes` (probe==1, extractColorMetadata==0 — the honest framing). `mockDb` gained an optional `array $colorRow = []` (existing 5-arg callers unaffected).
- `FfmpegRunnerToneMappingTest` (+2): column vs probe entry points agree for HDR meta (canonical graph); SDR meta + null meta → null.
- `stubColorMetadata()` intact; existing sub-step (b) HDR/SDR `ensureHlsJob` tests stay green (mockDb returns `[]` for `FROM media_streams` → `persistedColorMeta` null → probe path preserved).
- `--filter 'Transcode|Ffmpeg|ItemRepository'`: **OK (403 tests, 1540 assertions)** (+9). phpstan L9 `-c phpstan.neon.dist` on ALL changed src = **[OK] No errors**; changed test files = 0 NEW errors (the 2 in `TranscodeManagerTest` at lines ~1958/2311 are the SAME pre-existing offsetAccess/unusedUse confirmed on HEAD via `git stash`). phpcs PSR12 = **0 errors** (line-length WARNINGS only, matching the files' established house style).

...
{block name="title"}{$plugin.name} - Plugins{/block}
...
<section class="admin-plugin-detail">
    <header class="page-header">
        ...
        <p class="page-subtitle">
            ...
            type <code>{$plugin.type}</code> —
            entry <code>{$plugin.entry}</code>
        </p>
    </header>

    <dl class="plugin-meta">
        <dt>Installed at</dt>
        <dd>{$plugin.installed_at}</dd>
    ...
    <table class="settings-table">
        ...
        {foreach from=$settings item=row}
            <tr>
                ...
                <td>
                    {if $row.secret}
                        <span class="masked">{$row.value}</span>
                    {else}
                        {if $row.value === null}

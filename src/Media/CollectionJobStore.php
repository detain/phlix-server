...
/**
 * File-based job queue for TMDB box-set collection sync.
 *
 * One JSON file per pending job in a queue directory. Files are created when
 * jobs are enqueued (the scan path) and removed when dequeued/complete (the
 * worker). FIFO ordering by the timestamp line stored in each file. Bounded
 * queue size to prevent unbounded disk usage.
 *
 * Mirrors {@see SimilarityJobStore}, with one deliberate deviation: the queue
 * directory is created LAZILY on first enqueue, never in the constructor.
 * S439's zero-residue census fails the suite on any `/tmp/phlix_*` directory a
 * test run mints and does not sweep — and container-boot tests resolve this
 * store through the DI factories without ever enqueuing. A constructor-side
 * `mkdir()` would therefore leak `/tmp/phlix_collection_jobs` on clean CI
 * runners; a read-only store must remain side-effect-free.
 *
 * @since 0.38.0
 */
final class CollectionJobStore
{
    /** Default queue directory path */
    private const QUEUE_DIR = '/tmp/phlix_collection_jobs';
    ...
    /**
     * Enqueue a collection sync job.
     *
     * Idempotent - if the item is already enqueued, this is a no-op.
     *
     * @param CollectionJob $job The job to enqueue
     */
    public function enqueue(CollectionJob $job): void
    {
        $this->ensureQueueDir();
     ...
     *
     * @return CollectionJob|null The next job or null if queue empty
     */
    public function dequeue(): ?CollectionJob
    {
        ...
        try {
            ...
            return CollectionJob::fromArray($data);
        } catch (\Throwable) {
            // Corrupt file — skip
            return null;
        }

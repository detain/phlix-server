 ...
 *
 * Security guarantees (enforced through the generic {@see ImageResizer} this
 * class delegates every validation/resize/atomic-write to since S71 — the
 * public poster-cache API below is unchanged):
...
class ArtworkStorage
{
    ...
    /** Original size variant name (same string the generic service stores under). */
    public const ORIGINAL = ImageResizer::ORIGINAL;
    ...
    /**
     * Generic validation/resize/atomic-write service every keyed write runs
     * through (S71). Owned here with {@see self::$storageDir} as its base
     * directory so every item directory is identical to the pre-extraction one.
     */
    private ImageResizer $resizer;

    /**
     * @param string       $storageDir Artwork cache root (normalized with one trailing slash).
     * @param ImageResizer|null $resizer Optional injection for tests; when null a
     *                                service based on exactly {@see self::$storageDir}
     *                                is built. A non-null injection whose base
     *                                directory differs is rejected FAIL-FAST —
     *                                the storage path is a load-bearing invariant.
     */
    public function __construct(
        ...
        $resizer ??= new ImageResizer($this->storageDir);
        if ($resizer->baseDir() !== $this->storageDir) {
            throw new \InvalidArgumentException(
                'ImageResizer base directory does not match the artwork storage directory',
            );
        }
     ...
     * quality). The downloaded bytes are validated as a real PNG (the same
     * {@see ImageResizer::validateImageFile()} dimension-cap + real-MIME guards
     * the poster path uses) and then stored VERBATIM as `logo.png`, so the original
    ...
    public function downloadAndStoreLogo(string $itemId, string $logoPath): ?string
    {
        ...
        try {
            ...
            // via the generic keyed service (S71).
            $this->resizer->validateImageFile($tmpPath);
     ...
     *
     * Writing the original bytes preserves the alpha channel exactly. The generic
     * keyed store ({@see ImageResizer::storeBytes()}) is reused, which itself
     * routes through the atomic temp-then-rename writer so a concurrent reader
    ...
    private function storeLogoPng(string $itemId, string $tmpPath): ?string
    {
        ...
        return $this->resizer->storeBytes($itemId, self::LOGO_FILENAME, $bytes);
    }
     ...
     *
     * Delegates to {@see ImageResizer::atomicWriteVariant()}, which carries the
     * same temp-then-rename guarantee: a uniquely-named sibling temp file in the
     * SAME directory (so the final {@see rename()} is atomic on a single
     * filesystem), removed on any write or rename failure so no orphaned `.tmp`
     * files leak. `serveArtwork` streams these files via `withFile()`, and
     * {@see downloadAndStore()} only early-returns when ALL variants already
     ...
     * (and a mid-write ETag/Last-Modified) to a concurrent reader.
     *
    ...
    {
        return $this->resizer->atomicWriteVariant($variantFile, $jpegData);
    }
}

...
/**
 * Phlix media server component: Lastfm.
 *
...
namespace Phlix\Server\Integrations\Lastfm;

use Phlix\Common\Uuid;
use Workerman\MySQL\Connection;

/**
 * DB-backed implementation of {@see LastfmOAuthStateStore}.
 *
 * Stores Last.fm OAuth state entries in a database table instead of $_SESSION,
 * which causes state leakage between Workerman workers/requests.
 *
 * Uses the unified `oauth_state_store` table with provider = 'lastfm' and
 * data JSON containing {user_id}.
 *
 ...
 * @since 0.32.0
 */
final class DbLastfmOAuthStateStore implements LastfmOAuthStateStore
{
    /**
     * Provider identifier for Lastfm in the unified oauth_state_store table.
     */
    ...
    /**
     * Create a new DbLastfmOAuthStateStore instance.
     *
    ...
    /**
     * Persist a user ID keyed by the state value.
     *
     * @param string $state  Unique state identifier
     * @param string $userId Phlix user UUID who initiated the OAuth flow
     *
    ...
    public function put(string $state, string $userId): void
    {
        ...
        if ($result === false) {
            throw new \RuntimeException('Failed to persist Last.fm OAuth state to database');
        }
    }
     ...
     *
     * @return string|null The user ID if found and consumed, null otherwise
     */
    public function consume(string $state): ?string
    {
        // Atomic consume: SELECT within transaction, then DELETE
        $userId = $this->fetchAndDelete($state);
        if ($userId === null) {
     ...
     *
     * @return string|null The user ID if found, null otherwise
     */

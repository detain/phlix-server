 ...
 *
 * Sends formatted messages to Discord webhooks with:
 * - Embed color (RGB int)
 * - Title and description
 * - Thumbnail image
 * - Timestamp
 */
class DiscordPlugin extends AbstractNotificationPlugin
{
    ...
    public function send(WebhookEvent $event): bool
    {
        ...
        $this->getLogger()->debug('Discord notification sent', [
    ...
    protected function buildPayload(WebhookEvent $event): array
    {
        ...
        $thumbnail = $this->getThumbnailFromPayload($event->payload);
        if ($thumbnail !== '') {

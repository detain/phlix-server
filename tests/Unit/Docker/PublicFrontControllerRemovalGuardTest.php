 ...
 *
 * The deletion is pinned seven ways below: the file stays absent, and every
 * artifact that could re-front it — supervisord, the four Dockerfiles, every
 * compose file, the systemd unit — keeps fronting nothing, while the
 * installer's legacy-unit migration and public/'s surviving payloads (the SPA
 * bundle, the email/UI templates) stay in place. Recreating the file reddens
 * the first assertion; mutating any of the pinned properties reddens its own.
 *
...
final class PublicFrontControllerRemovalGuardTest extends TestCase
{
    ...
    public function testSupervisordRunsTheDaemonAndNearsNothingCgi(): void
    {

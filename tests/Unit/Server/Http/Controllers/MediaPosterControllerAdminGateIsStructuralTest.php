...
final class MediaPosterControllerAdminGateIsStructuralTest extends TestCase
{
     ...
     * {@see \Phlix\Server\Http\Router::callHandler()} exactly like a natively typed
     * one, and `phpstan analyse -c phpstan.neon.dist` (src/, level 9) reports
     * `[OK]` on it — measured in S323 phase 1. A native-type-only match therefore
     * left a whole ungated handler shape invisible to BOTH nets.
     *
    ...
    public function testEveryRequestHandlerIsGatedOrExplicitlyExempt(): void
    {
        ...
        foreach ($class->getMethods(ReflectionMethod::IS_PUBLIC) as $method) {
            ...
            if ($method->getDeclaringClass()->getName() !== MediaPosterController::class) {
                continue;
            }

...
class SessionControllerTest extends TestCase
{
    ...
    /**
     * The finish signal (default reached_end) finalizes via markAsWatched and
     * never via clearProgress. This is the caller S10 found missing.
     */
    public function testCompletePlaybackDefaultsToMarkAsWatched(): void
    {
        $sessionId = 'session-123';
        $userId = 'user-456';
        $mediaItemId = 'media-789';
        ...
        $request = new Request();
        $request->userId = $userId;
        $request->body = ['media_item_id' => $mediaItemId];
    ...
    /**
     * reached_end=false finalizes via clearProgress (the other dead caller S10
     * flagged) rather than marking the item watched.
     */
    public function testCompletePlaybackWithReachedEndFalseClearsProgress(): void
    {
        $sessionId = 'session-123';
        $userId = 'user-456';
        $mediaItemId = 'media-789';
        ...
        $request = new Request();
        $request->userId = $userId;
        $request->body = ['media_item_id' => $mediaItemId, 'reached_end' => false];
    ...
    public function testCompletePlaybackReturns404WhenSessionNotFound(): void
    {
        $sessionManager = $this->createMock(SessionManager::class);
        $sessionManager->method('getSession')->willReturn(null);
        ...
        $request = new Request();
        $request->userId = 'user-456';
        $request->body = ['media_item_id' => 'media-789'];

        $response = $controller->completePlayback($request, ['id' => 'missing-session']);

        $this->assertSame(404, $response->statusCode);
    }

    public function testCompletePlaybackReturns403WhenUserDoesNotOwnSession(): void
    {
        $sessionId = 'session-123';
        ...
        $request = new Request();
        $request->userId = 'user-456'; // not the owner
        $request->body = ['media_item_id' => 'media-789'];

 ...
 * {@see WebhookAdminController} is STRUCTURALLY required, and every handler is
 * behind it.
 *
 * ## The defect this file exists to make impossible
 *
 * `WebhookAdminController` used to hold
 ...
 * decision having been taken** — a fail-OPEN that downgraded all five
 * webhook-admin endpoints to auth-only, including the three that register,
 * rewrite and delete the outbound URLs this server signs and POSTs to.
 *
 * `requireAdmin()` calls `requireAuth()` first, so — unlike S323 phase 1's
 * `ThemeMediaController` — the hole never reached an ANONYMOUS caller. It reached
 * every logged-in one. Production always called the setter, so it was latent; that
 * is a property of the wiring, not of the class, and the wiring is exactly the
 * thing a future change can get wrong: PHP-DI's `autowire()` SKIPS optional
 * parameters, and this estate has already shipped silently-null dependencies that
 * way.
 *
 ...
 *     compare the middleware against null. This is what catches the three ways the
 *     optional shape can come back: a nullable property, a defaulted/omitted
 *     constructor parameter, or a re-introduced setter. A behavioural test alone
 *     would NOT catch a re-added setter, because a setter changes nothing until
 *     someone calls it — measured in S282's M2 mutation and reproduced in phase 1.
 *  2. **Behavioural** — all five handlers driven three ways (anonymous /
 ...
 */
final class WebhookAdminControllerAdminGateIsStructuralTest extends TestCase
{
    /**
     * Every admin-gated handler on {@see WebhookAdminController}, as
     ...
     * the fixture passes NO route params and an empty body, so `create()`,
     * `update()`, `delete()` and `test()` answer 400 on their own
     * missing-field/missing-id checks and `index()` answers 200 over the empty
     * webhook list. The gate emits only 401 and 403, so an admin arm that shows
     * the expected status proves the request reached the handler body rather than
     ...
     * detector that fails when the controller grows a sixth handler — gated OR
     * ungated.
     *
    ...
    {
        return [
            'GET    /api/v1/admin/webhooks'           => ['index', 200],
            'POST   /api/v1/admin/webhooks'           => ['create', 400],
            'PUT    /api/v1/admin/webhooks/{id}'      => ['update', 400],
            'DELETE /api/v1/admin/webhooks/{id}'      => ['delete', 400],
            'POST   /api/v1/admin/webhooks/{id}/test' => ['test', 400],
        ];
    }
     ...
     *
     * EMPTY, and that is the whole point on this controller: every endpoint it
     * exposes is an operator surface. An empty exempt list makes the classification
     * assertion STRICTER (every handler must be in the gated provider), never
     * laxer — it is not the "empty allow-list fails open" shape.
     *
     ...
     * dispatcher answers an empty webhook list.
     */
    private function makeController(): WebhookAdminController
    {
        ...
        // No refused or admitted request may ever reach a write or a delivery.
        $dispatcher = $this->createMock(WebhookDispatcher::class);
        $dispatcher->method('listWebhooks')->willReturn([]);
        $dispatcher->expects(self::never())->method('register');
        $dispatcher->expects(self::never())->method('update');
        $dispatcher->expects(self::never())->method('unregister');
        $dispatcher->expects(self::never())->method('dispatch');
        $dispatcher->expects(self::never())->method('dispatchToWebhook');
     ...
     * counts. Over-inclusion only ever forces a handler to be CLASSIFIED, which is
     * a review moment; under-inclusion is the fail-open.
     */
    ...
    {
        $ctor = (new ReflectionClass(WebhookAdminController::class))->getConstructor();
        self::assertNotNull($ctor, 'WebhookAdminController must declare a constructor');
        ...
        self::assertNotNull(
            $match,
            'WebhookAdminController::__construct() must take an AdminMiddleware. Setter injection '
    ...
    {
        $property = new ReflectionProperty(WebhookAdminController::class, 'adminMiddleware');

        $type = $property->getType();
        self::assertInstanceOf(
            ReflectionNamedType::class,
            $type,
            'WebhookAdminController::$adminMiddleware must carry a declared type'
        );
        self::assertSame(AdminMiddleware::class, $type->getName());
        self::assertFalse(
            $type->allowsNull(),
            'WebhookAdminController::$adminMiddleware must NOT be nullable — `?AdminMiddleware` is '
            . 'the exact shape S323 removed'
        );
        self::assertFalse(
            $property->hasDefaultValue(),
            'WebhookAdminController::$adminMiddleware must have no default value'
        );
    }
    ...
    {
        $class = new ReflectionClass(WebhookAdminController::class);
     ...
     *
     * A literal `new WebhookAdminController($dispatcher)` is a STATIC arity error,
     * so `phpstan analyse -c phpstan-tests.neon` (tests/, level 2) rejects the file
    ...
    public function testConstructingWithoutTheAdminMiddlewareIsAFatalError(): void
    {
        ...
        $class->newInstanceArgs([$dispatcher]);
    }
    ...
    {
        $method = new ReflectionMethod(WebhookAdminController::class, 'requireAdmin');
    ...
    // -----------------------------------------------------------------------
    // Net 2 — behavioural, over all five handlers
    // -----------------------------------------------------------------------

    /**
     * Arm 1 of 3 — anonymous. Expected 401 `auth.required`.
     *
     * The floor of the control, and on its own worth little: `requireAuth()` emits
     * the same 401 with no middleware at all. Arm 2 is the experiment.
     *
     * @dataProvider adminGatedHandlerProvider
     */
    public function testAnonymousCallerIsRefused(string $method): void
    {
        $response = $this->makeController()->{$method}(new Request(), []);
     ...
     * request with the handler's own 200/400 response; the distinct
    ...
    public function testAuthenticatedNonAdminIsRefusedOnTheAdminBranch(string $method): void
    {
        ...
        $request->body = ['name' => 'X', 'url' => 'https://example.com/h', 'secret' => 's'];

        $response = $this->makeController()->{$method}($request, ['id' => 'wh-1']);
     ...
     * outcome (200 over the empty list, or 400 for a missing id / missing fields).
     *
     ...
     * so it is structurally blind to a sixth handler added WITHOUT one — which is
     ...
     * below vacuously true, so the count is ASSERTED against a hardcoded 5 and the
    ...
    public function testEveryRequestHandlerIsGatedOrExplicitlyExempt(): void
    {
        ...
        foreach ($class->getMethods(ReflectionMethod::IS_PUBLIC) as $method) {
            ...
            if ($method->getDeclaringClass()->getName() !== WebhookAdminController::class) {
                continue;
            }
        ...
        self::assertCount(
            5,
            $handlers,
            'expected 5 public Request-taking handlers on WebhookAdminController; reflection '
        ...
        self::assertSame(
            $classified,
            $handlers,
            'every public Request-taking method of WebhookAdminController must be listed either in '
        ...
        foreach (self::adminGatedHandlerProvider() as $route => [$method]) {
            self::assertTrue(
                method_exists(WebhookAdminController::class, $method),
                "{$route} maps to WebhookAdminController::{$method}(), which must exist"
            );
        }
        ...
        self::assertSame(
            count($gated),
            $callSites,
            'WebhookAdminController has ' . $callSites . ' requireAdmin() call sites but '

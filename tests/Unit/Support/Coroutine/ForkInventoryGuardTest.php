 ...
 *
 * ⚠ Divergence S403 — PINNED: `Coroutine\System::exec` maps an unexecutable command
 * to `['code' => 127, 'output' => '']` while `shell_exec` yields `null` —
 * ChapterMarkerService::runCommand and FfmpegRunner::runProbeCommand /
 * runCoroutineAwareShellExec return '' vs null across arms. Benign for all
 * current callers; pinned rather than patched (no code-based rule aligns the
 * arms for every input without inventing a new contract). The contract test
 * `CoroutineShellExecDivergenceContractTest` pins the three-arm shape and
 * fails on drift.
 */

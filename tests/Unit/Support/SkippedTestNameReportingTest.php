 ...
 *
 * So the shapes are now RE-DERIVED from `./vendor/bin/phpunit` in a subprocess. The
 * one-skip shape used to be borrowed from
 * `tests/Unit/Admin/BackupManagerTest.php` — the estate's only unconditional
 * `markTestSkipped()` — and S452 turned that case into a real end-to-end backup, so
 * every shape is now generated here: the one-skip and two-skip shapes are throwaway
 * test classes with skipping test methods, the suite-skip shape a throwaway class with a
 * skipping `setUpBeforeClass()`. Minting them is also what stops this guard from
 * depending on a permanent skip surviving somewhere else in the estate. Hand-written
 * fixtures are kept ONLY for the shapes the binary cannot produce here: a `gh run view
 * --log` prefix, a whole-workflow log containing several runs, and format DRIFT.
 *
...
final class SkippedTestNameReportingTest extends TestCase
{
    ...
    /**
     * The class {@see self::writeOneTestSkipProbe()} mints: a test file with exactly ONE
     * skip, taken unconditionally in the body of `test_skips_exactly_one()` (the name in
     * {@see self::PROBE_SKIPPED_NAME}), so the real-binary expectations below do not
     * depend on MySQL, Chromium, FFI, mysqldump or anything else about the box.
     *
     * It used to be `tests/Unit/Admin/BackupManagerTest.php`, whose
     * `testCreateBackupGeneratesIdAndPath()` carried the estate's only unconditional
     * skip. S452 made that case run for real, and a guard of a reporting mechanism must
     * not be the reason a permanent skip is kept alive somewhere else in the estate — so
     * the fixture is written here, in the run's own scratch directory, beside the
     * suite-skip and two-skip probes.
     *
     ...
     */
    private const PROBE_CLASS = 'S345OneSkipProbeTest';

    private const PROBE_SKIPPED_NAME = 'Phlix\Tests\Scratch\S345OneSkipProbeTest::test_skips_exactly_one';

    /** The probe's green member — filtering to it produces a run that skips NOTHING. */
    private const PROBE_PASSING_NAME = 'test_runs_green';
    ...
    {
        $output = $this->runRealPhpunit([$this->writeOneTestSkipProbe()]);
        ...
        self::assertSame([self::PROBE_SKIPPED_NAME], $result['lines'], $result['stderr']);
    }

    public function test_a_real_testdox_run_is_reported_as_testdox_and_never_blamed_on_phpunit_xml(): void
    {
        $output = $this->runRealPhpunit([$this->writeOneTestSkipProbe(), '--testdox']);
    ...
    public function test_a_real_skipped_test_suite_is_named_rather_than_counted(): void
    {
        ...
        $mixed = $this->runRealPhpunit([$probe, $this->writeOneTestSkipProbe()]);
    ...
    public function test_ac2_the_mixed_testdox_suite_skip_shape_takes_the_testdox_verdict(): void
    {
        ...
        $mixed = $this->runRealPhpunit([$probe, $this->writeOneTestSkipProbe(), '--testdox']);
    ...
    public function test_a_real_colorized_run_is_parsed_rather_than_refused(): void
    {
        ...
        $output = $this->runRealPhpunit([$this->writeOneTestSkipProbe(), '--colors=always']);
    ...
    public function test_a_zero_skip_testdox_run_does_not_hijack_the_diagnosis_of_a_count_only_run(): void
    {
        $testdox = $this->runRealPhpunit([
            $this->writeOneTestSkipProbe(),
            '--testdox',
            '--filter',
            self::PROBE_PASSING_NAME,
        ]);
    ...
    {
        $testdox = $this->runRealPhpunit([$this->writeOneTestSkipProbe(), '--testdox']);
    ...
    public function test_ac1_a_multi_run_input_where_run_a_over_declares_and_run_b_under_declares_names_run_b(): void
    {
        ...
        $runB = $this->runRealPhpunit([$this->writeOneTestSkipProbe(), '--testdox']);
    ...
    public function test_the_documented_stdin_pipeline_yields_the_same_set_as_a_file_argument(): void
    {
        ...
        self::assertSame([self::PROBE_SKIPPED_NAME], $viaStdin['lines'], $viaStdin['stderr']);
        self::assertSame(
            $viaArgument['lines'],
            $viaStdin['lines'],
    ...
    {
        $with = $this->runRealPhpunitWithConfig($this->writeProbeConfig(true), [$this->writeOneTestSkipProbe()]);
        $without = $this->runRealPhpunitWithConfig($this->writeProbeConfig(false), [$this->writeOneTestSkipProbe()]);
        ...
        self::assertSame([self::PROBE_SKIPPED_NAME], $named['lines'], $named['stderr']);
     ...
     * message: the probe from {@see self::writeOneTestSkipProbe()} still skips exactly
     * one test.
     *
     * Round 2 of this step's review found three of those tests failing with "phpunit.xml is
     * not reaching it" when the likelier cause was the fixture's own skip disappearing —
     * which has nothing to do with `phpunit.xml`. That is precisely what S452 did to the
     * production file this used to borrow, and it is why the fixture is minted here: the
     * premise can no longer be edited away by unrelated work. `Skipped: N` is printed
     * whatever the config says (`SummaryPrinter.php`), so this check stays independent of
    ...
    private function assertTheFixtureStillSkipsExactlyOneTest(string $output): void
    {
        self::assertStringContainsString(
            'Skipped: 1',
            $output,
            self::PROBE_CLASS . ' no longer skips exactly one test, so every expectation '
            . 'in this test is measuring something else. This is NOT a phpunit.xml problem: '
            . 'the count is printed whatever the config says. Restore the single '
            . "unconditional markTestSkipped() in the probe. Output was:\n" . $output,
        );
    }
    ...
    /**
     * Writes the throwaway test class that supplies the one shape every real-binary
     * expectation here needs: EXACTLY ONE test-level `markTestSkipped()`, plus one green
     * member so a filtered run can produce the zero-skip class of input. Returns its path.
     *
     * Minted rather than borrowed (S452): this guard used to run over
     * `tests/Unit/Admin/BackupManagerTest.php` and its single permanent skip.
     */
    private function writeOneTestSkipProbe(): string
    {
        $path = $this->tmpDir . '/' . self::PROBE_CLASS . '.php';
        ...
        file_put_contents($path, <<<'PROBE'
            ...
            final class S345OneSkipProbeTest extends TestCase
            {
                public function test_runs_green(): void
                {
                    self::assertTrue(true);
                }

                public function test_skips_exactly_one(): void
                {
                    self::markTestSkipped('S345 probe: the one unconditional skip');
                }
            }
            PROBE);
